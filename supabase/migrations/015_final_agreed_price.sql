-- Add an immutable canonical price field that is written once when an offer
-- is accepted and never changes thereafter.  This eliminates historical drift
-- caused by the dynamic formula `requester_counter ?? counter_budget ?? budget`
-- which is vulnerable to post-acceptance mutations of any of those columns.
...
-- 2. Backfill existing accepted offers using the same priority chain.
UPDATE public.request_offers o
SET final_agreed_price = COALESCE(
  o.requester_counter,
  o.counter_budget,
  (SELECT r.budget FROM public.requests r WHERE r.id = o.request_id)
)
WHERE o.status = 'accepted'
  AND o.final_agreed_price IS NULL;

-- 3. Replace accept_offer_atomic so future acceptances also write final_agreed_price.
...
BEGIN
  ...
  -- 6. Capture the canonical price before any mutations.
  --    Priority: requester's counter > helper's counter > request budget.
